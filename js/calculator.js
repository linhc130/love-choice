...
if (Shipping == 0 || (Shipping !== Shipping)) {
        text += `<span class="Price">估算應付金額務必輸入請 代購件數 (最終應付金額仍由小幫手確認訂單方能成立)<span>`
    } else { 
...
🔷國際運費：1公斤300元</br>
🔷估算應付金額：<span class="totalPrice">${toCurrency(totalPrice)}</span> + <span class="Num">300*${num}</span>(重量預估${num}kg以內) = <span class="finalPrice">${toCurrency(finalPrice)}</span></p>` }
